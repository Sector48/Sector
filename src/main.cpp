...
unsigned int nStakeMinAge = 4 * 60 * 12 * 1; // 4 hours
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 100)
     ...
     else if(pindexBest->nHeight < 900)
     {
        nSubsidy = 200 * COIN;
        return nSubsidy + nFees;
     }
...
static const int64_t nTargetTimespan = 16 * 60;  // 20 mins
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "sector", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Sect 48 Sector";
        CTransaction txNew;
        txNew.nTime = 1440791444;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xb886e687f8f02da376f210590c4c8b2a0a54903c49c24d24968059342054bb13"));
        block.print();
